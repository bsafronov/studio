...
import { boolean, jsonb, pgEnum, pgTableCreator, text, timestamp, uuid, varchar } from "drizzle-orm/pg-core";
...
export const createdById = uuid('created_by_id').notNull().references(() => usersTable.id);
export const updatedById = uuid('updated_by_id').references(() => usersTable.id);
...
export const sheetColumnsTable = pgTable('sheet_columns', {
  ...base,
  sheetId: uuid('sheet_id').notNull().references(() => sheetsTable.id, {onDelete: 'cascade'}),
  name: varchar('name').notNull(),
...
export const sheetColumnsRelations = relations(sheetColumnsTable, ({one}) => ({
  createdBy: one(usersTable, {
